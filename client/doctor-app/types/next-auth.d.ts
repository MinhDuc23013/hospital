// NextAuth v5 type augmentation — extended with Keycloak claims.
// Keycloak JWT delivers roles at top-level `token.roles[]` (confirmed in plan validation).
// Doctor ID = Keycloak sub (confirmed: Keycloak sub === Doctor Service UUID).
import type { DefaultSession } from "next-auth";

declare module "next-auth" {
  interface Session {
    user: {
      /** Keycloak subject — used as Doctor ID in all API calls */
      id: string;
